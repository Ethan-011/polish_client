...
const Hero = () => {
  ...
  return <section id="home" className="relative h-screen w-full overflow-hidden flex items-center justify-center bg-metal-900" dir="rtl">
      ...
      <div className="absolute inset-0 bg-gradient-to-r from-metal-900/90 to-metal-800/80">
        ...
        <div className="absolute inset-0 overflow-hidden bg-transparent mx-0">
          {[...Array(20)].map((_, i) => <div key={i} className="absolute rounded-full bg-accent/20 animate-float" style={{
      ...
      <div className="container relative z-10 px-4 sm:px-6 lg:px-8 mx-auto">
        <div className="max-w-3xl mx-auto text-center">
          <div className={cn('transition-all duration-700 delay-300', isVisible ? 'opacity-100' : 'opacity-0 translate-y-10')}>
          ...
          <h1 className={cn('text-4xl sm:text-5xl md:text-6xl font-display font-bold text-white leading-tight mb-6', 'transition-all duration-700 delay-500', isVisible ? 'opacity-100' : 'opacity-0 translate-y-10')}>
          ...
          <p className={cn('text-xl text-metal-200 mb-10 max-w-2xl mx-auto', 'transition-all duration-700 delay-700', isVisible ? 'opacity-100' : 'opacity-0 translate-y-10')}>
            با بیش از ده سال تجربه در صنعت پرداخت کاری، بهترین کیفیت و زیبایی را برای فلزات شما به ارمغان می‌آوریم.
          </p>
          
          <div className={cn('flex flex-col sm:flex-row justify-center items-center gap-4', 'transition-all duration-700 delay-900', isVisible ? 'opacity-100' : 'opacity-0 translate-y-10')}>
      ...
      <div className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex flex-col items-center">
        <span className={cn('text-white/80 text-sm mb-2', 'transition-all duration-700 delay-1100', isVisible ? 'opacity-100' : 'opacity-0')}>
          برای دیدن بیشتر اسکرول کنید
        </span>
        <div className={cn('w-6 h-10 border-2 border-white/30 rounded-full flex justify-center p-1', 'transition-all duration-700 delay-1200 animate-pulse', isVisible ? 'opacity-100' : 'opacity-0')}>
